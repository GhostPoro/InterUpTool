...
public class Popups {
	...
	public static ToolOptions configWarning(ToolOptions opts, final String[] args) {
		Object[] options = { Text.STARTING_POPUP_TEXTS[1], Text.STARTING_POPUP_TEXTS[2], Text.STARTING_POPUP_TEXTS[3],
				Text.STARTING_POPUP_TEXTS[4] };
		int userConfigAnswer = -1;

		boolean requestAgain = true;

		String conf_popup_header  = Text.STARTING_POPUP_TEXTS[0].replace("$TOOL_NAME_SHORT$", Text.TOOL_NAME_SHORT).replace("$TOOL_VERSION$", Text.TOOL_VERSION);
		String conf_popup_message = Text.STARTING_POPUP_TEXTS[5].replace("$FFMPEG_ENCODER_APP_PATH$", "NOT_INITIALIZED");

		do {
			conf_popup_message = Text.STARTING_POPUP_TEXTS[5].replace("$FFMPEG_ENCODER_APP_PATH$", ((opts.FFMPEG_ENCODER_APP_PATH != null) ? opts.FFMPEG_ENCODER_APP_PATH : "NULL"));
			/*
			 * direct insertion of 'opts.FFMPEG_ENCODER_APP_PATH' inside 'replace' cause to
			 * crash if 'opts.FFMPEG_ENCODER_APP_PATH' == null, if no config file found, so
			 * premature initialization is required
			 */
			userConfigAnswer = JOptionPane.showOptionDialog(null, conf_popup_message, conf_popup_header,
			...
			if (userConfigAnswer == 0) { // Read config again
				...
				if (!opts.canProcess()) {
					String configText = TextProcessor.restoreVarsValues(TextProcessor.overrideVARs(
							Text.DEFAULT_CONFIGURATION_FILE_STRING, Configuration.VARS_TO_OVERRIDE_IN_CONFIG));
					...
					String configText = TextProcessor
							.restoreVarsValues(Utils.readFileToLine(Configuration.CORE_CONFIG_FILE_PATH));
					Utils.writeLinesToFile(Configuration.CORE_CONFIG_FILE_PATH, configText);
	...
	private static void updateTextArea(String text) {
		...
		CURRENT_runtimeErrorLogsPopup_TextArea.setPreferredSize(new Dimension(8192, 16000)); // otherwise scroll did not
																								// appear
